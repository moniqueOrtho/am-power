<template>
    <div class="crud-table">
        <div class="light1" v-if="tableSelect.title !== ''">
            <div class="am-container crud-table__title-box">
                <h4 class="text-h4 navigation__title mb-0">{{tableSelect.title}}</h4>
                <v-spacer></v-spacer>
                <div class="crud-table__select">
                    <v-select
                        v-model="tableSwitch"
                        :items="tableSelect.items"
                        :label="tableSelect.label"
                        @change="tableMustChange"
                    ></v-select>
                </div>
            </div>
        </div>
...
export default {
    ...
    created () {
        if(this.tableSelect.defaultInput) this.tableSwitch = this.tableSelect.defaultInput;
    ...
    methods: {
        ...
        if(this.headers.findIndex(x => x.value === 'rank') > -1) {
            this.ownItems = this.rankedItems(data);
        } else {
            this.ownItems = [...data];
        }
